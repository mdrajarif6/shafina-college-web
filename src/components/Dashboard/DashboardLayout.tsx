...
export function DashboardLayout({ lang, notices, setNotices, applications, setApplications, students, setStudents, results, setResults, attendance, setAttendance, settings, setSettings, onLogout }: DashboardLayoutProps) {
    ...
    <div className="flex h-screen bg-slate-50 font-sans">
      ...
      <div className="w-64 bg-slate-950 text-white flex flex-col shadow-xl z-10">
        ...
        <div className="p-4 border-t border-slate-800">
          <button
            onClick={onLogout}
            className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-white/5 hover:bg-rose-600 rounded-xl transition-all text-sm font-bold text-slate-300 hover:text-white cursor-pointer"
          >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top Header */}
        <header className="bg-white border-b border-slate-200 p-4 flex items-center justify-between shadow-sm z-0">
          <h1 className="text-xl font-bold text-slate-800">
            {menuItems.find((m) => m.id === currentView)?.[lang === 'EN' ? 'labelEN' : 'labelBN']}
          </h1>
          <div className="flex items-center gap-4">
            <span className="text-sm font-semibold text-slate-600 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">
              Admin User
            </span>
          </div>
        </header>
